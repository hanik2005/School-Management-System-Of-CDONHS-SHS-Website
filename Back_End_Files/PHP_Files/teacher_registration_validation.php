...
function validateTeacherEnrollment($connection, $data) {
    ...
    // ===============================
    // VALIDATE LRN (Numbers Only) - For teachers this could be employee ID if needed
    // ===============================
    // Uncomment if teacher has LRN field
    // if (isset($data['lrn'])) {
    //     $lrn = $data['lrn'];
    //     if (!preg_match('/^[0-9]+$/', $lrn)) {
    //         $errors[] = "Employee ID must contain numbers only.";
    //     }
    // }
    ...
    if (isset($data['firstName'], $data['lastName'])) {
        ...
        // Check for exact match in teacher_applications
        $checkDuplicateName = $connection->prepare(
            "SELECT teacher_application_id, first_name, last_name 
        ...
        if ($checkDuplicateName->num_rows > 0) {
            $errors[] = "A teacher with the same name (first name, last name, middle name, and extension name) already exists in the system.";
        }
    }
